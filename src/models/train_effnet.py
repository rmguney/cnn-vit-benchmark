...
class EfficientNetB0ForClassification(pl.LightningModule):
    def __init__(self, num_classes=10, learning_rate=0.001):
        ...
        # Load EfficientNet-B0 from torchvision and adapt it for CIFAR-10
    ...
    def forward(self, x):
        return self.model(x)  # Only return class logits
...
def main():
    ...
    model = EfficientNetB0ForClassification(num_classes=10, learning_rate=learning_rate)
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        filename=f"EfficientNetB0ForClassification_{current_time}_best",
        save_top_k=1
    )
    checkpoint_callback_epoch = ModelCheckpoint(
        every_n_epochs=1,
        dirpath=saved_models_dir,
        filename=f"EfficientNetB0ForClassification_{current_time}_epoch{{epoch}}"
    )
    lr_monitor = LearningRateMonitor(logging_interval="epoch")
    csv_logger = CSVLogger(logs_dir, name=f"EfficientNetB0ForClassification_{current_time}")
